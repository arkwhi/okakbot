...
def E():
    try:
        A = C.TeleBot(TOKEN)
        D(A)  # стандартные хендлеры
        F(A)  # кастомные хендлеры

        from telebot import types
        commands = [
            types.BotCommand("start", "Начать работу с ботом"),
            types.BotCommand("help", "Показать список команд"),
            types.BotCommand("set_spok", "Сохранить пожелание спокойной ночи"),
            types.BotCommand("spok", "Отправить пожелание"),
            types.BotCommand("hello", "Поприветствовать"),
            types.BotCommand("bye", "Попрощаться"),
            types.BotCommand("ogo", "Сказать 'ого'"),
            types.BotCommand("id", "Показать свой Telegram ID"),
            types.BotCommand("whoami", "Информация о пользователе"),
            types.BotCommand("set_quote", "Сохранить цитату"),
            types.BotCommand("quote", "Показать цитату"),
        ]
        A.set_my_commands(commands)
...
''' def E():
    try:A=C.TeleBot(TOKEN);D(A);F(A) 
    from telebot import types 
        commands = [types.BotCommand("start", "Начать работу с ботом"),
    ...
    A.set_my_commands(commands);
    B.info('Бот запускается...');A.    remove_webhook();A.polling(none_stop=True,interval=0)
